...
class SopirResponse {
  ...
  factory SopirResponse.fromJson(Map<String, dynamic> json) {
    List<SopirModel> dataList = [];
    if (json['data'] != null) {
      List<dynamic> list = json['data'].cast<dynamic>();
      dataList = list.map((i) => SopirModel.fromJson(i)).toList();
    }

    var linkList = json['links'] as List? ?? [];
    List<Link> linkDataList = linkList.map((i) => Link.fromJson(i)).toList();

    return SopirResponse(
